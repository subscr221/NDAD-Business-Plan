<!-- converted from Defense_Platform_Workbook_Explanation.docx -->
...
Source workbook: Defense_Platform_Business_Plan_Financial_V2.xlsx
...
Assumptions defines global drivers: tax, WACC, working-capital days, learning curves, bank guarantees, MAT, R&D amortization life, manufacturing overhead, warranty.
Platform defines shared platform NRE investment and module maturity over 10 years.
R&D_Buckets splits Platform NRE into research expensed, development capitalised, and development expensed / not eligible.
ReuseMatrix maps each vertical to the five shared platform modules and calculates how much vertical NRE is avoided through reuse.
V1_AESA to V6_MilAI contain supporting product-level unit economics: ASP, BoM, units, revenue, COGS, margins.
Backlog is the consolidated revenue engine. Bookings and opening backlog flow into deliveries. Deliveries become recognized revenue.
Revenue pulls recognized revenue from Backlog and scales BoM COGS using the V-sheet unit economics.
Headcount, R&D_NRE, Capex, and Opex build the cost base.
P&L combines revenue, COGS, R&D/NRE, S&M, G&A, opex, D&A, interest, tax, NOL, MAT, and the R&D tax bridge.
CashFlow converts the P&L into CFO, CFI, FCF, financing flows, and cash balances.
BalanceSheet links cash, working capital, PP&E, intangibles, debt, equity, retained earnings, and checks A = L + E.
Funding provides equity/debt inflows and checks minimum cash compliance.
KPIs and BreakEven summarize the model outputs.
...
Revenue, Headcount, Capex, Opex, Funding, Backlog, V1 to V6, R&D_NRE, Platform, R&D_Buckets, and ReuseMatrix use Y1 in column D.
...
How it is built: it pulls outputs from P&L, CashFlow, Backlog, Funding, Headcount, Capex, BalanceSheet, R&D_NRE, and Assumptions.
...
Feeds R&D_Buckets, ReuseMatrix, and R&D_NRE.
...
Connections:
Depends on Platform, Assumptions, R&D_NRE, and P&L.
Feeds R&D_NRE rows 14, 16, 17, and 18.
...
Connections:
Depends on Platform.
Feeds R&D_NRE and KPI counterfactual savings.
...
R&D amortization life (C58): feeds R&D_Buckets, R&D_NRE, P&L, CashFlow, and BalanceSheet.
### V1_AESA
...
Connections:
Depends on Assumptions.
Feeds R&D_NRE for R&D personnel.
...
Row 14: weighted Platform R&D capitalisation rate from R&D_Buckets.
Row 16: capitalised development additions from R&D_Buckets.
Row 17: book-expensed Platform R&D from R&D_Buckets.
Row 18: amortisation of capitalised R&D from R&D_Buckets.
...
D&A from Capex plus capitalized R&D amortization from R&D_NRE (Y1=9.77, peaks Y6=92.69, Y10=54.56 INR Cr).
...
CFI includes gross capex, change in restricted cash, and capitalized R&D additions from R&D_Buckets.
CFF pulls equity and debt flows from Funding.
FCF = CFO + CFI.
Cash balance = prior cash + FCF + CFF.
Connections:
Depends on P&L, Assumptions, Backlog, Capex, R&D_NRE, and Funding.
...
Connections:
Depends on CashFlow, Backlog, P&L, Capex, R&D_NRE, Funding, and Assumptions.
Feeds KPIs.
...
Rows 17-23 pull fixed-cost stack from R&D_NRE, P&L, and D&A.
...
Connections:
Depends on V1-V6, R&D_NRE, P&L, CashFlow, Funding, and Assumptions.
...
The collateral schedule tapers over time. Y1 to Y2 require 100% cash collateral; Y3 drops to 50%, Y4 to 30%, and Y5 onwards holds at 20%. This is consistent with real practice at relationship banks once the company has demonstrated repeat performance.
BG issuance fee is set at 1.0% per annum (reduced from the more typical 1.5%) because the Rs 150 Cr land and factory asset on the balance sheet provides the issuing bank with physical security beyond the cash collateral.
...
Cash consumption (Y1 to Y3): Cumulative FCF declines from -359.92 INR Cr in Y1 to a peak deficit of -1,061.78 INR Cr in Y3. Every line item works against cash in these years - heavy working-capital deployment, peak capex, restricted cash building against initial BGs, and recognized revenue still well below run-rate cost.
Inflection (Y4): Recognized revenue (855.20 INR Cr) finally outpaces the absolute level of working-capital deployment plus capex, and FCF turns marginally positive at +2.99 INR Cr. This is the operational crossover point. Cumulative FCF still sits at -1,058.79 INR Cr at this point, so the deficit has not yet been recovered.
Cash accumulation (Y5 onwards): From Y5, FCF compounds rapidly. Cumulative FCF crosses zero between Y6 and Y7 (Y6 ends at -221.28; Y7 ends at +704.55), meaning the business has fully repaid its cumulative cash deficit and is now in net cash-generation territory.
...
Even on the most punitive basis (10-year FCF only, no TV, 18% WACC), the project produces +747.15 INR Cr of NPV against 1,100 INR Cr of equity deployed. This clears the hurdle rate without requiring any belief in terminal value or any softening of the discount rate. It is the floor case.
The real investor return comes from the terminal value. A defence platform that has reached 2,347.09 INR Cr of Y10 EBITDA is not a project that ends - it is a going concern that an acquirer (a listed Indian defence major, a US or European prime, a strategic PE buyer) will pay 10 to 14x EBITDA for. The 12x case is the central estimate, not the upside case.
Including TV, the equity NPV is 6,128.50 INR Cr against ~1,100 INR Cr deployed - a value-creation multiple of 5.5x in present-value terms after 18% WACC. The investor IRR rises to the 40 to 44% range. This is the actual investment thesis.
The 10-year FCF-only NPV (747.15 INR Cr) and the equity NPV including TV (6,128.50 INR Cr) bracket the answer for risk-averse and risk-neutral readers. A scout who wants the most conservative number takes 747. An LP or strategic acquirer comparing this against other defence platform investments takes 6,128 - and rationally, because every comparable transaction in Indian defence has been priced off forward EBITDA multiples, not truncated FCF.
...
To change revenue:
Edit Backlog bookings rows 20-25.
Edit Backlog delivery rates rows 82-87.
Review Revenue, P&L, CashFlow, and Funding.
To change product unit economics:
Edit V-sheet ASP, BoM, or unit assumptions.
Review Revenue BoM COGS scaling.
Review gross margins in P&L and BreakEven.
To change platform leverage:
Edit Platform NRE and maturity (rows 6-10 and 15-19).
Edit module mix in ReuseMatrix if the vertical architecture changes.
Edit Assumptions!C26 platform reuse multiplier for a one-cell scenario lever.
Review R&D_NRE and KPI NRE savings.
To change working capital/funding stress:
Edit Assumptions DSO, DPO, inventory days, advance rate, PBG, BG fee, and collateral.
Review Backlog, CashFlow, restricted cash, and Funding gap.
To change accounting treatment:
Edit R&D_Buckets research and development capitalisation percentage inputs.
Edit Assumptions amortization life (C58) if the capitalised development asset life changes.
Review R&D_NRE, P&L, CashFlow, BalanceSheet, and the R&D bucket KPI rows.
Recalculate the workbook in Excel or LibreOffice, because formula values must be refreshed after structural accounting changes.
To stress-test NPV:
Change Assumptions!C7 (WACC) - 18% is the base; try 14% for an at-listing-comparable case.
Shift the EBITDA inflection by editing Backlog delivery rates (delay Y4 by raising V4/V5 thresholds or lowering V1/V2 delivery percentages).
Edit Assumptions BoM learning curve (C12) and BoM floor (the 0.92 and 0.70 base values) to test margin compression.
Stretch Assumptions DSO from 180 to 240 to model worst-case MoD payment behaviour.
Re-run the KPIs Terminal Value memo block to see how NPV including TV moves.
